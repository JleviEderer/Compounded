Refined HeatMapGrid component with updated color scheme and grid styling.
```
...
export default function HeatMapGrid({ cells, gridType, onCellClick, getIntensityColor }: HeatMapGridProps) {
  ...
  const getColor = (value: number | null) => {
    ...
    if (value < 0) {
      if (value <= -4) return NEG_HI;
      if (value <= -2) return NEG_MID;
      return NEAR_ZERO;
    }
    if (value > 0) {
      if (value >= 4) return POS_HI;
      if (value >= 2) return POS_MID;
      return NEAR_ZERO;
    }
    return NO_DATA;
  };
