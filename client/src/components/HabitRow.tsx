...
import { SUCCESS_RATE_THRESHOLDS, SuccessColour, rateToColour } from '@/constants/successRate';
...
const colourClass = (colour: SuccessColour) => {
  switch (colour) {
    case 'green':
...
// Helper function to get label for colour
const labelForColour = (colour: SuccessColour) => {
  switch (colour) {
    case 'green':
      return 'On track';
    case 'yellow':
...
export default function HabitRow({ habit, logs, onLogHabit, isToday = false, showSavedFlash = false, successRate }: HabitRowProps) {
  ...
  const successColour = rateToColour(successRate);
    ...
    >
      <div className="flex items-start justify-between min-w-0 gap-3">
        ...
        <div className="flex items-start space-x-2 flex-shrink-0 mt-1">
         <span className={`text-xs font-medium min-w-[35px] text-right ${colourClass(successColour)}`}>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0```typescript
// client/src/constants/successRate.ts
...
export function rateToColour(ratePct: number): SuccessColour {
    if (ratePct >= SUCCESS_RATE_THRESHOLDS.green) {
        return 'green';
    } else if (ratePct >= SUCCESS_RATE_THRESHOLDS.yellow) {
        return 'yellow';
    } else {
        return 'red';
    }
